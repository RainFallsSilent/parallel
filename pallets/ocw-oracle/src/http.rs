            ...
            for currency_id in currency_vec.iter() {
                ...
                for data_source in data_source_vec.iter(){
                    if let Some(url_bytes) = OcwOracleRequestUrl::<T>::get(currency_id, data_source) {
                        let url = str::from_utf8(&url_bytes).map_err(|_| <Error<T>>::ParseUrlError)?;
                        let resp_bytes = Self::fetch_from_remote(url).map_err(|e| {
                            debug::error!("fetch_from_remote error: {:?}", e);
                            <Error<T>>::HttpFetchingError
                        })?;
                        let payload_detail: TickerPayloadDetail = get_ticker::<T>(currency_id.clone(), data_source.clone(), resp_bytes)?;
                        res.push(payload_detail);
                    }
                }
                sp_io::offchain::sleep_until(sp_io::offchain::timestamp().add(sp_core::offchain::Duration::from_millis(HTTP_INTERVAL)));
            }
